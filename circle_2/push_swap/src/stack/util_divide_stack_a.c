...
/*   Updated: 2022/07/11 19:22:34 by sooyokim         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "push_swap.h"

int	divide_stack_a_pivot1(t_head *a, t_head *b, int *pivot_cnt1, t_buf *buf)
{
...
int	divide_stack_a_pivot2(t_head *a, t_head *b, int *pivot_cnt2, t_buf *buf)
{
	a->head->cluster_cnt = 0;
	push_from_to(a, b, buf, 'b');
	reverse(b, buf, 'b');
...
int	divide_stack_a_pivot3(t_head *a, int *pivot_cnt3, t_buf *buf)
{
	a->head->cluster_cnt = 0;
	reverse(a, buf, 'a');
...
int	pivot_cnt_set_a(t_head *a, t_head *b, t_pc *pivot_cnt, t_buf *buf)
{
	int	i;
...
int	divide_stack_a(t_head *a, t_head *b, t_pivot pivot, t_buf *buf)
{
	...
	while (--i > 0)
	{
		...
		if (temp <= pivot.pivot1)
			if (!divide_stack_a_pivot1(a, b, &pivot_cnt->pivot1_cnt, buf))
				return (0);
		if (temp > pivot.pivot1 && temp <= pivot.pivot2)
			if (!divide_stack_a_pivot2(a, b, &pivot_cnt->pivot2_cnt, buf))
				return (0);
		if (temp > pivot.pivot2)
			if (!divide_stack_a_pivot3(a, &pivot_cnt->pivot3_cnt, buf))
				return (0);
	}
	if (!pivot_cnt_set_a(a, b, pivot_cnt, buf))
		return (0);
	return (1);
}
